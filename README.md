...
Occasionally clients have their Cloudflare sitting behind another system that is a Proxy.  In that case, the Adapter might not receive the visitor's IP address where it's expected, which can interfere with normal operation.  You can check this on the Logs tab in the Queue-Fair Portal if it is enabled on your account, or contact Queue-Fair support and we can check this for you, so we recommend that you do tell us that you have installed the Cloudflare Adapter after you have completed your install, or after making changes to your system that could afffect this.
